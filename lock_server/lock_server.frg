...
pred doGrant[h: Host] {
    (all h1: DistributedSystem.hosts-h |  h1.holdsLock = HoldsLockFalse) 
    h.holdsLock = HoldsLockTrue
...
pred sendMsg[e: Int] {
    ...
    one m: Message | {
        m.msgEpoch < m.msgEpoch' 
        and
        m.msgEpoch' = e 
        and
        (some h: DistributedSystem.hosts | m.dest' = h) 
        and
...
pred doAccept[h: Host] {
    some m: Network.msg | (recvMsg[m] and m.dest = h and h.epoch' = m.msgEpoch and m = m')
    // h.epoch <= h'.epoch
        ...
        some step: HostSteps| { 
            {
                ...
                (some h: DistributedSystem.hosts | doGrant[h] 
                    // { h.holdsLock = HoldsLockTrue and doGrant[h]} 
                )
            }
            or 
            {
